...
void printEnv(std::string filename, std::vector<std::vector<std::vector<int> > > env, int scale)
{
    ...
    else if (filename.find(".abc") != std::string::npos)
    {
        outfile << "P2\n"
                << scale *  env[0].size() << " " << scale * env.size() << "\n" <<"1\n";
        //things are repeated to scale them up (the image is too small!)
...
void printWind(std::vector<std::vector<std::vector<double> > > U,
    ...
    std::cout << "print wind 2\n";
    std::cout << U[0][0].size() << " " << U[0].size() << " " << U.size() << "\n";
...
bool planeIntersectsDistance(Eigen::Vector3d& n, const Eigen::Vector3d& planePoint,const Eigen::Vector3d& triangle_vertex_0)
{
    double distance;
    double A = n(0);
    double B = n(1);
    double C = n(2);
    double x0 = triangle_vertex_0(0);
    double y0 = triangle_vertex_0(1);
    double z0 = triangle_vertex_0(2);
    double x1 = planePoint(0);
    double y1 = planePoint(1);
    double z1 = planePoint(2);

    distance = absolute(A*(x1-x0)+B*(y1-y0)+C*(z1-z0))/std::sqrt(A*A+B*B+C*C);
                     ...
                     bool parallel, bool debug)
{
    ...
    for(const Eigen::Vector3d &vec : cube){
        ...
        if(debug)
        {
            std::cout << "proyectionInTriangle " << proyectionInTriangle<<" "<<alpha<<" "<<beta<<" "<<gamma<<"\n";
            std::cout <<"w"<< w << "\n";
            std::cout <<"u x w"<< u.cross(w) << "\n"; 
            std::cout <<"(u x w).n" << u.cross(w).dot(n) << "\n";
            std::cout <<"w x v"<< w.cross(v) << "\n"; 
            std::cout <<"(w x v).n"<< w.cross(v).dot(n) << "\n"; 
        }
        idx++;
    }
    ...
    if((!anyProyectionInTriangle) && debug)
    {
    ...
    if(debug)
    {
        ...
        std::cout <<"intersect? "<< planeIntersects(n, query_point,cube) << "\n";
        std::cout <<"intersect d? "<< planeIntersectsDistance(n, query_point,triangle_vertex_0) << "\n";
    }
    //return (anyProyectionInTriangle && (parallel||planeIntersects(n, query_point,cube)));
    return (anyProyectionInTriangle && (parallel||planeIntersectsDistance(n, query_point,triangle_vertex_0)));
    //return (parallel||planeIntersects(n, query_point,cube));
    //return anyProyectionInTriangle;
    //return parallel;
    //return planeIntersects(n, query_point,cube);
}
...
void occupy(std::vector<std::vector<std::vector<int> > >& env,
    ...
    for(int i= 0;i<points.size();i++){
        //if(normals[i][1] == 0 && normals[i][1] == 0 && normals[i][2] == -1)
        //{
        ...
        for (int row = min_x; row <= max_x && row < env[0].size(); row++)
        {
            for (int col = min_y; col <= max_y && col < env.size(); col++)
            {
                ...
                {
                    //std::cout << i <<" "<< row<<" " << col <<" "<< height << "\n";
                    if((i == 4)&&(((row == min_x)&&(col == min_y+3))||((row == min_x+3)&&(col == min_y)))&&(height==min_z))
                    {
                        debug = true;
                        std::cout << "debug true\n";
                    }
...
void parse(std::string filename, std::vector<std::vector<std::vector<int> > >& env, int val){
    ...
    if (FILE *file = fopen(filename.c_str(), "r"))
    {
        ...
        std::cout<< "[parse] File " << filename << " does not exist\n";
    }
    ...
    while (line.find("endsolid")==std::string::npos)
        {
            ...
            std::getline(infile, line);
            //std::cout<< "normals " << i << " " << normals[i][0] << " " << normals[i][1] << " " << normals[i][2] << "\n";
            for(int j=0;j<3;j++){
                std::getline(infile, line);
                ...
                //std::cout<< "points " << j << " " << points[i][j][0] << " " << points[i][j][1] << " " << points[i][j][2] << "\n";
            }
...
void findDimensions(std::string filename){
    if (FILE *file = fopen(filename.c_str(), "r"))
    {
        ...
        std::cout<< "[findDimensions] File " << filename << " does not exist\n";
    }
	...
	while (std::getline(infile, line))
	{
		if (line.length()!=0)
		{
			for (int i = 0; i < 6; i++)
			{
                ...
                //std::cout << "openfoam 4.1\n";
				v[i] = atof(line.substr(0, pos).c_str());
                //std::cout << "openfoam 4.2\n";
				line.erase(0, pos + 1);
			}
            //std::cout << "openfoam 4.3\n";
...
void fill2D(int x, int y, int z, std::vector<std::vector<std::vector<int> > >& env, int val, int empty){
    std::queue<Eigen::Vector3i> q;
    q.push(Eigen::Vector3i(x, y, z));
    env[x][y][1]=val;
    while(!q.empty()){
...
int main(int argc, char **argv){
    ...
    std::cout<<"Filling...\n";
    //printEnv(boost::str(boost::format("%s/occupancy0.abc") % output.c_str()), env, 10);

    //std::cout<<"Done filling...\n";
    fill2D((empty_point_x-env_min_x)/cell_size,
        (empty_point_y-env_min_y)/cell_size,
        (empty_point_z-env_min_z)/cell_size, 
        env, 3, 0);
    //std::cout<<"Cleaning...\n";
    //printEnv(boost::str(boost::format("%s/occupancy1.abc") % output.c_str()), env, 10);
    clean(env);

    //std::cout<<"Cleaning done...\n";
    //printEnv(boost::str(boost::format("%s/occupancy2.abc") % output.c_str()), env, 10);
    ...
    private_nh.param<int>("number_of_models_rand", numModels_rand, 2); // number of CAD models
    
    std::vector<std::string> CADfiles_rand;     
    for(int i = 0; i< numModels_rand; i++){
        std::string paramName_rand = boost::str( boost::format("model_rand_%i") % i); //each of the stl models
        std::string filename_rand;
        private_nh.param<std::string>(paramName_rand, filename_rand, "");
        CADfiles_rand.push_back(filename_rand.c_str());
    }
    std::vector<std::vector<std::vector<int> > > env_rand(ceil((env_max_y-env_min_y)*(roundFactor)/(cell_size*(roundFactor))),
                                                 std::vector<std::vector<int> >(ceil((env_max_x - env_min_x)*(roundFactor)/(cell_size*(roundFactor))),
                                                                                    std::vector<int>(ceil((env_max_z - env_min_z)*(roundFactor)/(cell_size*(roundFactor))), 0)));

    for (int i=0;i<numModels_rand; i++){
        parse(CADfiles_rand[i], env_rand, 1);
    }
    //empty_point_x = 3;
    //empty_point_y = 6;
    //empty_point_z = 1;
    fill((empty_point_x-env_min_x)/cell_size,
        ...
        env_rand, 3, 0);
    clean(env_rand);
    printEnv(boost::str(boost::format("%s/OccupancyRandGrid3D.csv") % output.c_str()), env_rand, 1);
    std::cout<<"Done rand...\n";
